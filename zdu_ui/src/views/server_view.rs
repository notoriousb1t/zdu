...
use iced::widget::{button, column, container, responsive, row, rule, scrollable, text, Column, Row, Space};
...
pub fn view<'a, Message: Clone + 'a>(
    ...
    let status_text = match session {
        ...
        SessionState::Open { port, clients } => text(format!(" - ✔️ Server Status: Running on Port {} ({} clients)", port, clients)).size(18),
    };
...
fn build_all_items<'a, Message: 'a>(items: &HashMap<u8, u8>) -> Vec<Element<'a, Message>> {
    let get_val = |offset: u8| -> u8 { items.get(&offset).copied().unwrap_or(0) };
    
    let get_bool = |offset: u8| -> String {
        if get_val(offset) > 0 { "Yes".to_string() } else { "No".to_string() }
    };

    vec![
        build_item_row("Arrow:", match get_val(0x02) {
            1 => "Wood".to_string(),
            2 => "Silver".to_string(),
            _ => "None".to_string(),
        }).into(),
        ...
        build_item_row("Boomerang:", if get_val(0x1E) > 0 { "Magical".to_string() } else if get_val(0x1D) > 0 { "Wood".to_string() } else { "None".to_string() }).into(),
        build_item_row("Bow:", get_bool(0x03)).into(),
        build_item_row("Candle:", match get_val(0x04) {
            1 => "Blue".to_string(),
            2 => "Red".to_string(),
            _ => "None".to_string(),
        }).into(),
        ...
        build_item_row("Potion:", match get_val(0x07) {
            1 => "Blue".to_string(),
            2 => "Red".to_string(),
            _ => "None".to_string(),
        }).into(),
        ...
        build_item_row("Ring:", match get_val(0x0B) {
            1 => "Blue".to_string(),
            2 => "Red".to_string(),
            _ => "None".to_string(),
        }).into(),
        ...
        build_item_row("Sword:", match get_val(0x00) {
            1 => "Wood".to_string(),
            2 => "White".to_string(),
            3 => "Magical".to_string(),
            7 => "Fairy".to_string(),
...
fn inventory_view<'a, Message: 'a>(items: &HashMap<u8, u8>) -> Element<'a, Message> {
